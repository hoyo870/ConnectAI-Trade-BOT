"""
[실험] RSI 폭 축소 + SL 여유 테스트 (2026-06-11)

변경 파라미터 (기준: scripts/backtest_antifragile.py):
- dt_rsi_lo: 22 → 25   (하락추세 롱 진입 RSI 상향)
- ut_rsi_lo: 35 → 38   (상승추세 롱 진입 RSI 상향)
- ut_rsi_hi: 78 → 75   (상승추세 숏 진입 RSI 하향)
- trail_atr_init:  0.5 → 1.0
- trail_atr_tight: 0.8 → 1.2

Usage:
  python temp/scripts/45_rsi_sl_test.py --mode 2026 --coin all
  python temp/scripts/45_rsi_sl_test.py --mode both --coin all
  python temp/scripts/45_rsi_sl_test.py --mode random --coin btc
"""
...
def run_antifragile(
    ...
    dt_rsi_lo = 25,   # 하락추세: 롱 진입  (기준 22)
    dt_rsi_hi = 65,   # 하락추세: 숏 진입
    rg_rsi_lo = 30,   # 횡보:     롱 진입
    rg_rsi_hi = 70,   # 횡보:     숏 진입
    ut_rsi_lo = 38,   # 상승추세: 롱 진입  (기준 35)
    ut_rsi_hi = 75,   # 상승추세: 숏 진입  (기준 78)
    ...
    # Trailing Stop
    trail_atr_init  = 1.0,    # 초기 trailing stop 거리 (ATR 배수)  (기준 0.5)
    trail_atr_tight = 1.2,    # 피라미딩 후 tight trailing (ATR 배수)  (기준 0.8)
    # 기타
...
def main():
    ...
    parser.add_argument("--trail-init", type=float, default=1.0)
    parser.add_argument("--trail-tight",type=float, default=1.2)
    parser.add_argument("--add-step",   type=float, default=0.5)
    parser.add_argument("--dt-lo",  type=float, default=25)
    parser.add_argument("--dt-hi",  type=float, default=65)
    parser.add_argument("--rg-lo",  type=float, default=30)
    parser.add_argument("--rg-hi",  type=float, default=70)
    parser.add_argument("--ut-lo",  type=float, default=38)
    parser.add_argument("--ut-hi",  type=float, default=75)
    ...
    cfg = dict(
        ...
        dt_rsi_lo       = args.dt_lo,
        dt_rsi_hi       = args.dt_hi,
        rg_rsi_lo       = args.rg_lo,
        rg_rsi_hi       = args.rg_hi,
        ut_rsi_lo       = args.ut_lo,
        ut_rsi_hi       = args.ut_hi,
    )
    ...
    for coin in coins:
        ...
        print(f"  trail_init={args.trail_init}  trail_tight={args.trail_tight}  add_step={args.add_step}")
        print(f"  RSI dt({args.dt_lo}/{args.dt_hi})  rg({args.rg_lo}/{args.rg_hi})  ut({args.ut_lo}/{args.ut_hi})")
        print(f"{'█'*66}")
